...
1. Instale o GNOME Extensions (se ainda não tiver):
```bash
sudo apt install gnome-shell-extension-prefs  # Ubuntu/Debian
sudo dnf install gnome-extensions-app         # Fedora
```

2. Abra o app "Extensões" e ative "Global Menu"

## 🎯 Como Usar

1. **Automático**: Assim que ativada, a extensão começa a funcionar automaticamente
2. **Visualização**: O nome do aplicativo ativo aparece na barra superior
3. **Acesso ao Menu**: Clique no nome do app para ver o menu
4. **Interação**: Navegue pelos menus como em qualquer menu nativo
